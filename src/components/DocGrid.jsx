   ...
   <div>
      ...
      <div>
         ...
         <table>
            ...
            {medicos.map((medico)=><tr key={medico.id}><td>{medico.name}</td><td>{medico.lastname}</td><td>{medico.specialty}</td><td>{medico.street}</td><td>{medico.number}</td><td>{medico.city}</td><td><img src={medico.url} alt="imagen_perfil" /></td>{/*<td>{medico.address.street} {medico.address.number } {medico.address.city }</td>*/} <button onClick={()=>erasealert(medico.id)}>eliminar</button> <Link to={`/doctors/edit/${medico.id}`} > <button>  Editar</button> </Link> </tr> )} 
