...
return(
   <div className="prestador" >
         ...
         <table className='listadoMedicos responsive-table'>
          ...
          <thead>
            ...
            {medicos.map((medico)=><tr key={medico.id}><td className='datos'>{medico.name}</td><td className='datos'>{medico.lastname}</td><td className='datos'>{medico.specialty}</td><td className='datos'>{medico.street}</td><td className='datos'>{medico.number}</td>
            <td className='datos'>{medico.city}</td><td><img src={medico.url} alt="imagen_perfil" /></td>{/*<td>{medico.address.street} {medico.address.number } {medico.address.city }</td>*/} 
            <button style={{marginRight:5}} className="btn edit" onClick={()=>erasealert(medico.id)}>Eliminar</button> <Link to={`/doctors/edit/${medico.id}`} > <button className="btn registro">  Editar</button> </Link> </tr> )} 
